// Bir dəfəlik test: hər qoşulmuş TikTok hesabına 1 slideshow (dərhal dərc).
// İşə salma (GitHub Actions və ya lokal):  SITE_BASE=... ZERNIO_API_KEYS=... node post-once.mjs
...
if (!keys.length) { console.error("ZERNIO_API_KEYS yoxdur"); process.exit(1); }
...
for (const key of keys) {
  ...
  for (const acc of accounts) {
    ...
    const body = {
      content: pick(r, content.captions), hashtags: pick(r, content.hashtagSets),
      mediaItems: files.map((f) => ({ type: "image", url: `${base}${f}`, mimeType: "image/png" })),
      platforms: [{ platform: "tiktok", accountId: acc._id }], publishNow: true,
    ...
    const pr = await fetch(`${ZERNIO}/v1/posts`, {
      method: "POST",
      headers: { Authorization: `Bearer ${key}`, "Content-Type": "application/json", "x-request-id": `test-${Date.now()}-${acc._id}` },
      body: JSON.stringify(body),
    });
    const txt = await pr.text();
    if (pr.ok) console.log(`OK  @${acc.username || acc._id}: ${files.length} slaydlıq post`);
    else { console.log(`XETA @${acc.username || acc._id}: ${pr.status} ${txt.slice(0, 300)}`); fails++; }
  }
}
